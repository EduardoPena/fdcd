...
public class TwoColEvidenceInversion implements DCEnumeration {
	...
	public TwoColEvidenceInversion(TwoColumnPredicateSpace pSpace, EvidenceSet eviSet) {
		this(pSpace, eviSet, (TranslatingTreeSearch) null);
		this.startBitsets.add(LongBitSet.FACTORY.create());

	}

	private TwoColEvidenceInversion(TwoColumnPredicateSpace pSpace, EvidenceSet eviSet, TranslatingTreeSearch tree) {
		...
		for (ColumnPredicateGroup group : pSpace.getOneColspace().getPredicateGroups()) {
		...
		for (TwoColumnPredicateGroup pGroup : pSpace.getTwoColCatpredGroups()) {
			IBitSet bitset = LongBitSet.FACTORY.create(toBitSet(pGroup.getPids()));
			bitsetList.add(bitset);
		}
	...
	/* for modulo approach */
	public TwoColEvidenceInversion(TwoColumnPredicateSpace pSpace, Set<BitSet> evidenceBitSets, IndexedPredicate p,
			Set<Integer> predicatets2Remove) {
		this(pSpace, evidenceBitSets, (TranslatingTreeSearch) null, p, predicatets2Remove);
		this.startBitsets.add(LongBitSet.FACTORY.create());
	}

	private TwoColEvidenceInversion(TwoColumnPredicateSpace pSpace, Set<BitSet> evidenceBitSets,
			TranslatingTreeSearch tree, IndexedPredicate p, Set<Integer> preds2Remove) {
		...
		for (TwoColumnPredicateGroup pGroup : pSpace.getTwoColCatpredGroups()) {
			...
			if (pGroup.getPredicates().contains(p))
				continue;
	...
	/* for hybrid approach */
	public TwoColEvidenceInversion(TwoColumnPredicateSpace pSpace, Set<BitSet> evidenceBitSets,
			Set<Integer> predicatets2Remove) {
	...
	private TwoColEvidenceInversion(TwoColumnPredicateSpace pSpace, Set<BitSet> evidenceBitSets,
			TranslatingTreeSearch tree, Set<Integer> preds2Remove) {
	...
	private BitSet toBitSet(Set<Integer> pids) {
	...
	public boolean isTrivialDC(BitSet dc) {
		...
		List<IndexedPredicate> preds = new ArrayList<>();
		for (int p1 = dc.nextSetBit(0); p1 >= 0; p1 = dc.nextSetBit(p1 + 1)) {
			preds.add(pSpace.getPredicateById(p1));
		}

		for (int i = 0; i < preds.size(); i++) {
			for (int j = i + 1; j < preds.size(); j++) {
				if (preds.get(i).hasEquivalentOperands(preds.get(j)))
					return true;
