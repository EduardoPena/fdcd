...
public class TwoColINCS implements DCEnumeration {
	...
	public TwoColINCS(TwoColumnPredicateSpace predicateSpace, EvidenceSet evidenceSet) {
		this(predicateSpace, evidenceSet, true, true);
	}

	public TwoColINCS(TwoColumnPredicateSpace predicateSpace, EvidenceSet evidenceSet, boolean parallel) {
		this(predicateSpace, evidenceSet, true, parallel);
	}

	public TwoColINCS(TwoColumnPredicateSpace predicateSpace, EvidenceSet evidenceSet, boolean checkMinimals,
			boolean parallel) {
	...
	public TwoColINCS(TwoColINCS base) {
	...
	public Set<BitSet> searchDCs() {
		...
		stream.forEach(i -> {
			...
			TwoColINCSExecutor search = new TwoColINCSExecutor(currPreds, pSpace,
					getModuloEvi(currEvis, pSpace.getPredicateById(pid), predicatets2Remove),false);

			Set<BitSet> partialDCs = search.searchDCs();

			for (BitSet cover : partialDCs) {
